...
class MF
attr_accessor :whole, :fracNum, :fracDen
  def initialize(whole, fracNum, fracDen)
    @whole = whole
    @fracNum = fracNum
    @fracDen = fracDen
  end # initialize(whole, fracNum, fracDen)  
  def self.zero
    MF.new(0, 0, 1)
  end # self.zero
  ...
  def self.newNr(value,base=32)
    ...
    result = mfvalue.rebase(base)
    puts "finished rebase with #{result.inspect}"
    result
  end # self.newNr
